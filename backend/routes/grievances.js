...
function sqliteToView(row) {
  return {
    ...
    due_at: row.due_at || row.received_at,
  ...
  try {
    ...
    const inserted = await run(
      ...
      [
        ...
        b.receivedDate || new Date().toISOString(),
        req.user.id
      ]
    );
  ...
  try {
    ...
    try {
      await transporter.sendMail({
        from: fromUser,
        to: targetEmails,
        subject: `[MWCA ${reminder ? 'Overdue Reminder' : 'Grievance Referral'}] ${row.reference_number} - ${row.subject}`,
        text: [
          `Dear ${targetDept},`,
          '',
          note || `Please assess and process official MWCA grievance ${row.reference_number}.`,
          '',
